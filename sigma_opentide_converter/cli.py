...
def convert(input_file: Path, output_file: Path, target: str, verbose: bool):
    ...
    try:
        ...
        success = converter.convert_file(str(input_file), str(output_file), target_systems)
...
@click.option('--verbose', '-v', is_flag=True, help='Enable verbose output')
def batch(input_dir: Path, output_dir: Path, target: str, verbose: bool):
...
def _show_manual_completion_warning():
    ...
    click.echo("   - Scheduling configuration (frequency, lookback, cron)", fg='yellow')
    click.echo("   - Impacted entities (device, user, mailbox)", fg='yellow')
    click.echo("   - Review and adjust queries as needed", fg='yellow')
    click.echo("   Look for '# MANUAL COMPLETION REQUIRED' comments in the output", fg='yellow')
