...
class SigmaToOpenTideConverter:
    ...
    def generate_query(self, sigma_rule: SigmaRule, target_system: str) -> tuple[str, bool]:
        """Generate query using sigma-cli for Defender and PySigma for Splunk
        
        Returns:
            tuple: (query_string, success_status)
        """
        try:
            if target_system == 'defender_for_endpoint':
                query = self._generate_defender_query_with_sigma_cli(sigma_rule)
                # Check if it's an error message
                if query.startswith("# Error"):
                    return query, False
                return query, True
            else:
                query = self._generate_splunk_query_with_pysigma(sigma_rule)
                # Check if it's an error message
                if query.startswith("# "):
                    return query, False
                return query, True
            ...
            return f"# Error generating query for {target_system}: {str(e)}", False
    ...
    def create_opentide_yaml(self, sigma_rule: SigmaRule, target_systems: Optional[List[str]] = None) -> tuple[Dict[str, Any], Dict[str, bool]]:
        """Create OpenTide YAML structure from Sigma rule
        
        Returns:
            tuple: (yaml_data, generation_status) where generation_status is a dict of {system: success_bool}
        """
        ...
        for target_system in target_systems:
            query, success = self.generate_query(sigma_rule, target_system)
            generation_status[target_system] = success
        ...
        return yaml_data, generation_status
    ...
    def convert_file(self, input_path: str, output_path: str, target_systems: Optional[List[str]] = None, 
                     interactive: bool = True) -> tuple[bool, Dict[str, bool]]:
        """Convert a single Sigma rule file to OpenTide format
        
        Args:
            input_path: Path to input Sigma rule file
            output_path: Path to output OpenTide YAML file
            target_systems: List of target systems to generate queries for
            interactive: Whether to prompt user on failures
            
        Returns:
            tuple: (overall_success, generation_status_dict)
        """
        try:
            ...
            yaml_data, generation_status = self.create_opentide_yaml(sigma_rule, target_systems)
            
            # Check if any queries failed
            failed_systems = [sys for sys, status in generation_status.items() if not status]
            successful_systems = [sys for sys, status in generation_status.items() if status]
            
            if failed_systems:
                print(f"\n⚠ WARNING: Query generation failed for: {', '.join(failed_systems)}")
                
                if successful_systems:
                    print(f"✓ Query generation successful for: {', '.join(successful_systems)}")
                    
                    if interactive:
                        # Ask user if they want to proceed with only successful queries
                        response = input("\nDo you want to proceed with only the successful queries? (y/n): ").strip().lower()
                        
                        if response == 'y':
                            # Remove failed configurations
                            for failed_system in failed_systems:
                                if failed_system in yaml_data.get('configurations', {}):
                                    del yaml_data['configurations'][failed_system]
                            
                            print(f"\nProceeding with configurations for: {', '.join(successful_systems)}")
                        else:
                            print("\nConversion cancelled by user.")
                            return False, generation_status
                    else:
                        # In non-interactive mode, remove failed configurations automatically
                        for failed_system in failed_systems:
                            if failed_system in yaml_data.get('configurations', {}):
                                del yaml_data['configurations'][failed_system]
                else:
                    print("✗ All query generations failed. Cannot proceed.")
                    return False, generation_status
            ...
            if failed_systems:
                print(f"\n⚠ Note: Configurations for {', '.join(failed_systems)} were excluded due to errors")
            
            print("\n⚠ IMPORTANT: Manual completion required for:")
    ...
    def convert_directory(self, input_dir: str, output_dir: str, target_systems: Optional[List[str]] = None,
                          interactive: bool = False) -> int:
        """Convert all Sigma rule files in a directory
        
        Args:
            input_dir: Input directory containing Sigma rules
            output_dir: Output directory for converted files
            target_systems: List of target systems to generate queries for
            interactive: Whether to prompt user on failures (default False for batch)
        ...
        for yaml_file in yaml_files:
            ...
            success, generation_status = self.convert_file(str(yaml_file), str(output_file), target_systems, interactive)
            
            if success:
                # Check if all targets succeeded
                all_succeeded = all(generation_status.values())
                if all_succeeded:
        ...
        print(f"  ✓ Fully successful: {successful_conversions}/{len(yaml_files)}")
        if partial_conversions > 0:
            print(f"  ⚠ Partial success: {partial_conversions}/{len(yaml_files)}")
        if failed_conversions > 0:
            print(f"  ✗ Failed: {failed_conversions}/{len(yaml_files)}")
        print(f"{'='*60}")
        
        return successful_conversions + partial_conversions